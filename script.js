...
document.addEventListener('DOMContentLoaded',function(){
  ...
  const uniqueTraits = [
    ...new Map(traits.map(t => [t.name, t])).values()
  ];
  ...
  const quartermasterTrait = uniqueTraits.find(trait => trait.name === "Quartermaster");
  ...
  const traitPool = (pool || uniqueTraits).slice();
  const selected = [];
  const usedNames = new Set();
  let remaining = availablePoints;

  while (remaining > 0 && selected.length < maxSlots && traitPool.length > 0) {
    const affordable = traitPool.filter(trait => trait.cost <= remaining && !usedNames.has(trait.name));
  ...
  function randomizeTraits() {
    ...
    const pool = uniqueTraits.filter(trait => !forcedTraits.some(f => f.name === trait.name));
    ...
    const names = selectedTraits.map(t => t.name);
    const dupes = names.filter((n, i) => names.indexOf(n) !== i);
    console.log("Selected traits:", names);
    console.log("Duplicate names this roll:", dupes);
  }
  ...
  if (weapon.isDual) {
    ...
    if (i === 1) {
      img.style.marginLeft = "-140px";   // tweak -12 / -20px to taste
    }
  } else {
